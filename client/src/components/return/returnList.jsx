...
import { removeReturnList } from "../../features/return/returnSlice";

const ReturnList = () => {
  let returnListTotal = useSelector((state) => state.return.returnListTotal);
  let returnList = useSelector((state) => state.return.returnList);

  let [pageNo, setPageNo] = useState(1);
  let [perPage, setPerPage] = useState(20);
  let [searchKeyword, setSearchKeyword] = useState("0");
  let [searchValue, setSearchValue] = useState("");
  let totalPage = Math.ceil(returnListTotal / perPage);
  let searchClick = () => {
  ...
  useEffect(() => {
    listReturnRequest(pageNo, perPage, searchKeyword);
  ...
  let deleteCustomerHandleClick = async (id) => {
    ...
    if (result.isConfirmed) {
      let rst = await deleteReturnRequest(id);
      if (rst) {
        store.dispatch(removeReturnList(id));
      }
    }
  };
