...
const ReturnReport = () => {
  let returnReport = useSelector((state) => state.report.returnReport);
  ...
  const handleClick = () => {
    ...
    if (!IsEmpty(fromDate)) {
      ...
      returnReportRequest(fromDate, toDate);
    }
  };

  const OnExport = (exportType, data) => {
    const fileName = "Return-Report";
    if (data.length > 0) {
      let ReportData = [];
      data.map((item) => {
        let listItem = {
              ...
              <div className="card-body">
                <div className="row">
                  <div className="col">
                    <h6>
                      Total:{" "}
                      {returnReport.total[0]["totalAmount"] >= 0 ? (
                        <CurrencyFormat
                          value={returnReport.total[0]["totalAmount"]}
                          displayType={"text"}
                    ...
                    <button
                      onClick={() => OnExport("csv", returnReport.rows)}
                      className="btn btn-sm my-2 btn-success"
                    >
                      Download CSV
                    </button>
                    <button
                      onClick={() => OnExport("xls", returnReport.rows)}
