...
import { deleteCategory, listCategory } from "../../APIRequest/categoryApi";
import moment from "moment";
import { removeFromCategoryList } from "../../features/category/categorySlice";

const CategoryList = () => {
  const categoryList = useSelector((state) => state.category.categoryList);
  const categoryTotal = useSelector((state) => state.category.categoryTotal);

  const [pageNo, setPageNo] = useState(1);
  const [perPage, setPerPage] = useState(20);
  const [searchKeyword, setSearchKeyword] = useState("0");
  let [searchValue, setSearchValue] = useState("0");
  let totalPage = Math.ceil(categoryTotal / perPage);
  ...
  useEffect(() => {
    listCategory(pageNo, perPage, searchKeyword);
  ...
  const deleteCustomerHandleClick = async (id) => {
    ...
    if (confirm.isConfirmed) {
      ...
      if (result) {
        store.dispatch(removeFromCategoryList(id));
      }
    }
  };
