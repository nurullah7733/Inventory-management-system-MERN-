...
const SupplierList = () => {
  let { total, list } = useSelector((state) => state.supplier);
  ...
  useEffect(() => {
    SupplierListApi(pageNo, perPage, searchKeyword);
  ...
  let deleteCustomerHandleClick = async (id) => {
    ...
    if (result.isConfirmed) {
      let rst = await deleteSupplier(id);
      if (rst) {
        store.dispatch(removeSupplier(id));
      }
    }
  };
