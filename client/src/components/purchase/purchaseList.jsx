...
import { removePurchaseList } from "../../features/purchase/purchaseSlice";

const ProductList = () => {
  let purchaseListTotal = useSelector(
    (state) => state.purchase.purchaseListTotal
  );
  let purchaseList = useSelector((state) => state.purchase.purchaseList);

  let [pageNo, setPageNo] = useState(1);
  let [perPage, setPerPage] = useState(20);
  let [searchKeyword, setSearchKeyword] = useState("0");
  let [searchValue, setSearchValue] = useState("");
  let totalPage = Math.ceil(purchaseListTotal / perPage);
  let searchClick = () => {
  ...
  useEffect(() => {
    listPurchaseRequest(pageNo, perPage, searchKeyword);
  ...
  let deleteCustomerHandleClick = async (id) => {
    ...
    if (result.isConfirmed) {
      let rst = await deletePurchaseRequest(id);
      if (rst) {
        store.dispatch(removePurchaseList(id));
      }
    }
  };
